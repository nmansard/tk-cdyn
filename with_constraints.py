...
reset_button = tk.Button(optimFrame, text="Reset", command=constraintsManager.resetAndDisp)
reset_button.pack(side=tk.LEFT, padx=10, pady=10)
optim_button = tk.Button(optimFrame, text="Optim", command=constraintsManager.computeConstrainedConfig)
...
class CheckboxDisplayConstraintCmd:
    ...
    def __call__(self):
        ...
        for n in self.gname:
            self.viz.viewer.gui.setVisibility(n,'ON' if self.boolVar.get() else 'OFF')
...
for i,cm in enumerate(cassie.full_constraint_models):
    ...
    active_constraint_cmd = CheckboxConstraintCmd(active_constraint_var,cm,cassieFrame.project)
